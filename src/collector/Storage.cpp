...
void Storage::save_group_history(std::vector<GroupHistoryEntry> && history, time_t timestamp)
{
...
void Storage::update_group_structure()
{
    ...
    for (const GroupHistoryEntry & entry : m_group_history) {
        ...
        if (it != m_groups.end()) {
            ...
            BH_LOG(app::logger(), DNET_LOG_ERROR, "History database contains record for "
...
void Storage::merge_nodes(const Storage & other_storage, bool & have_newer)
{
    ...
    while (other != other_storage.m_nodes.end()) {
...
void Storage::merge_hosts(const Storage & other, bool & have_newer)
{
    ...
    while (other_it != other.m_hosts.end()) {
